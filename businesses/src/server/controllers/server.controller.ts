...
export class Server extends ServerEntityWithEntranceController<Business> {
  ...
  private registerCommands(): void {
    this.RegisterAdminCommand(
      ...
      (source: number, args: any[]) => {
        ...
        console.log(
          ...
          `has set a new deposit position for business ID ${
            business.id
          } to X: ${business.depositX.toFixed(
            4
          )}, Y:${business.depositY.toFixed(4)}, Z:${business.depositZ.toFixed(
            4
          )}!`
        );
    ...
    this.RegisterAdminCommand(
      ...
      (source: number, args: any[]) => {
        if (args.length < 3) {
          console.log(
            'Syntax: /createbusiness <business-type> <price> <level>!'
          );
          return;
        }
        ...
        if (
          ...
          this.entities.find(
            (business: Business) =>
              business.name === BUSINESS_INTERIORS[businessType].name
          )
        ) {
    ...
    RegisterCommand(
      ...
      (source: number) => {
        ...
        if (!business) {
          global.exports['chat'].addMessage(
            source,
            "Couldn't find a valid business."
          );
          return;
        }
    ...
    RegisterCommand(
      'businessmenu',
      (source: number) => {
        const virtualWorld: number = GetEntityRoutingBucket(
          GetPlayerPed(source)
        );
        const business: Business = this.entities.find(
          (_business: Business) => _business.id === virtualWorld
        );
              ...
              { key: 'Pet Status', value: 'Happy' },
              {
                ...
                value:
                  business.sellingPrice > 0
                    ? `$${numberWithCommas(business.sellingPrice)}`
                    : 'Not for Sale',
              },
              {
                key: 'Value',
                value: '$' + numberWithCommas(business.firstPurchasePrice),
              },
    ...
    onNet(
      ...
      async (_source: number) => {
        ...
        if (
          ...
          business.owner = global.exports['authentication'].getPlayerInfo(
            _source,
            'name'
          );
          ...
          if (result) {
            ...
            TriggerClientEvent(
              `${GetCurrentResourceName()}:business-purchased`,
              _source,
              business
            );
          } else {
  ...
  protected syncWithClients(client?: number): void {
    ...
    if (client) {
      ...
      const businessesOwnedByPlayer: Business[] = this.entities.filter(
