...
export class Client extends ClientWithUIController {
    ...
    private updateUIData(business: Business): void {
        ...
        let title: string = '';
        if (isUnowned) {
            title = `Business for Sale! (Price: $${numberWithCommas(business.firstPurchasePrice)}) (#${business.id})`;
        } else {
        ...
        SendNuiMessage(JSON.stringify({
            ...
            description: 'Businesses grant you passive income, even when you\'re offline. Business income can also be shared with players, by making them partners.',
            resource: GetCurrentResourceName(),
            buttons: [
                {
                    title: 'Purchase',
                    subtitle: `Purchase this business for $${numberWithCommas(businessPrice)}.`,
                    icon: 'attach_money',
    ...
    protected onIncomingUIMessage(eventName: string, eventData: any): void {
        ...
        if (eventName === 'buttonclick') {
            ...
            switch (data.buttonId) {
                case 0: {
                    TriggerServerEvent(`${GetCurrentResourceName()}:request-purchase-business`);
                    break;
                }
            }
        }
        ...
        if (eventName === 'dialogbuttonclick') {
            ...
            switch (data.dialogId) {
                case 'purchase_unowned_business': {
                    TriggerServerEvent(`${GetCurrentResourceName()}:confirm-purchase-business`, GetPlayerServerId(PlayerId()));
                    break;
                }
            }
        }
    ...
    private assignListeners(): void {
        onNet(`${GetCurrentResourceName()}:db-send-entities`, (businesses: Business[]) => {
            this.clearMarkers();
            this.clearBlips();
            ...
            businesses.forEach((business: Business) => {
                ...
                this.createBlips([{
                    ...
                    title: !isOwnedByMe ? `Business - ${business.name} - ${!isUnowned ? 'Owned' : 'Unbought'}` : `Business - ${business.name} - Yours`,
                    pos: [business.entranceX, business.entranceY, business.entranceZ]
                }]);
                ...
                this.createActionPoints(
                    {
                        ...
                        action: () => {
                            ...
                            AddTextComponentSubstringPlayerName(`Press ~INPUT_PICKUP~ to ${isOwnedByMe || amIPartner ? 'enter the business.' : 'interact.'}${businessPrice > 0 ? `~n~Press ~INPUT_SPECIAL_ABILITY_SECONDARY~ to buy for $${numberWithCommas(businessPrice)}.` : ''}`);
                ...
                if (!this.actionPoints.find((actionPoint: ActionPoint) => { actionPoint.pos[0] === business.exitX && actionPoint.pos[1] === business.exitY && actionPoint.pos[2] === business.exitZ })) {
                    this.createActionPoints(
                        {
                            ...
                            action: () => {
                                ...
                                BeginTextCommandDisplayHelp("STRING");
                                AddTextComponentSubstringPlayerName("Press ~INPUT_PICKUP~ to exit the business.~n~Press ~INPUT_INTERACTION_MENU~ to open up the menu.");
                                EndTextCommandDisplayHelp(0, false, true, 1);
