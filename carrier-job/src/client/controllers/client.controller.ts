import { CARRIER_MARKERS, CARRIER_QUICKSTART_POSITIONS } from '../../shared/positions';
...
export class Client extends ClientWithUIController {
    ...
    private addControllerListeners(): void {
        onNet(`${GetCurrentResourceName()}:begin-route`, async (deliveryPosition: { X: number, Y: number, Z: number }, shouldSpawnVehicle?: boolean) => {
            if (shouldSpawnVehicle) {
                await this.createVehicleAsync(GetHashKey('Mule'), CARRIER_QUICKSTART_POSITIONS[0].pos[0], CARRIER_QUICKSTART_POSITIONS[0].pos[1], CARRIER_QUICKSTART_POSITIONS[0].pos[2], 45.061977386475, true, true, true);
            }
            
            const deliveryPoint: number = this.createWaypoint([deliveryPosition.X, deliveryPosition.Y, deliveryPosition.Z], 'Job - Carrier - Delivery Point', 69, 652);
        ...
        onNet(`${GetCurrentResourceName()}:pickup-route`, async (deliveryPosition: { X: number, Y: number, Z: number }) => {
            const deliveryPoint: number = this.createWaypoint([deliveryPosition.X, deliveryPosition.Y, deliveryPosition.Z], 'Job - Carrier - Pickup Point', 81, 652);
    ...
    private updateUIData() {
        ...
        SendNuiMessage(JSON.stringify({
            type: 'update',
            title: 'Carrier Job',
            description: "Carriers deliver local cargo to stores and other businesses. Businesses rely on Carriers' cargo in order to keep running.",
