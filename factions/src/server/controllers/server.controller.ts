...
export class Server extends ServerEntityWithEntranceController<Faction> {
  ...
  public getOnlineFactionMembers(internalId: string): FactionMember[] {
    const faction: Faction = this.entities.find((_faction: Faction) => _faction.internalId === internalId);

    if (faction) {
      return faction.members.filter((factionMember: FactionMember) => factionMember.onlineId != null);
    }
  ...
  public getAllFactionMembers(internalId: string): FactionMember[] {
    return this.entities.find((faction: Faction) => faction.internalId === internalId)?.members || [];
  }

  private registerCommands(): void {
    this.RegisterAdminCommand('removefaction', 6, (source: number, args: any[]) => {
        let faction: Faction = this.getClosestEntityOfSameTypeToPlayer(source);
        if (args && args[0]) {
            faction = this.getEntityByDBId(Number(args[0])) || this.getClosestEntityOfSameTypeToPlayer(source);
        }
        ...
        console.log(global.exports['authentication'].getPlayerInfo(source, 'name'), 'removed a faction!');
        this.removeEntity(faction);
    }, false);

    this.RegisterAdminCommand('createfaction', 6, (source: number, args: any[]) => {
        if (args.length < 3) {
    ...
    RegisterCommand('factionmenu', (source: number) => {
        const faction: Faction = this.entities.find((_faction: Faction) => _faction.internalId === 'taxi'); // TODO: Use actual faction internalId
  ...
  private registerListeners(): void {
    onNet('authentication:player-authenticated', (playerAuthenticated: number, playerInfo: Player) => {
      this.entities.forEach((faction: Faction) => {
        const authenticatedPlayerFactionMemberData: FactionMember = faction.members.find((_factionMember: FactionMember) => _factionMember.id === playerInfo.id);
    ...
    onNet('authentication:player-logout', (playerLoggedOut: number) => {
      this.entities.forEach((faction: Faction) => {
        const loggedOutPlayerFactionMemberData: FactionMember = faction.members.find((_factionMember: FactionMember) => _factionMember.onlineId === playerLoggedOut);
