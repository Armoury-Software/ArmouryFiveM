...
export class Client extends ClientWithUIController {
    ...
    private assignListeners(): void {
        onNet(`${GetCurrentResourceName()}:db-send-entities`, (factions: Faction[]) => {
            this.clearMarkers();
            this.clearBlips();
            ...
            factions.forEach((faction: Faction) => {
                ...
                this.createActionPoints(
                    {
                        ...
                        action: () => {
                            ...
                            AddTextComponentSubstringPlayerName(`Press ~INPUT_PICKUP~ to enter the faction HQ.`);
                ...
                if (!this.actionPoints.find((actionPoint: ActionPoint) => { actionPoint.pos[0] === faction.exitX && actionPoint.pos[1] === faction.exitY && actionPoint.pos[2] === faction.exitZ })) {
                    this.createActionPoints(
                        {
                            ...
                            action: () => {
                                ...
                                BeginTextCommandDisplayHelp("STRING");
                                AddTextComponentSubstringPlayerName("Press ~INPUT_PICKUP~ to exit the faction HQ.~n~Press ~INPUT_INTERACTION_MENU~ to open up the menu.");
                                EndTextCommandDisplayHelp(0, false, true, 1);
