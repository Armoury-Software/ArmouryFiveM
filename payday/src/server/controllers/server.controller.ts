  ...
  private assignListeners(): void {
    this.paydayInterval = setInterval(() => {
      if (new Date().getMinutes() === 0) {
        ...
        const players: number[] = global.exports['authentication'].getAuthenticatedPlayers();

        const taxes: Map<string, number> = new Map([
          ['electricity', businesses.find((business: any) => business.name === 'Electricity Company')?.productPrice || 100],
          ['gas', businesses.find((business: any) => business.name === 'Gas Company')?.productPrice || 100],
          ['phone', businesses.find((business: any) => business.name === 'Phone Company')?.productPrice || 100]
        ]);

        players.forEach((player: number) => {
          const housePrice: number = global.exports['houses'].getEntities().find((house: any) => house.owner === GetPlayerName(player))?.firstPurchasePrice || 0;
          const houseTax: number = Number(0.0005 * housePrice);
          const businessPrice: number = businesses.find((business: any) => business.owner === GetPlayerName(player))?.firstPurchasePrice || 0;
          const businessTax: number = Number(0.0005 * businessPrice);
          const salary: number = Math.floor(1000 + Math.random() * 1000);
          const interest: number = Math.max(0, Math.floor(0.000001 * Number(global.exports['authentication'].getPlayerInfo(player, 'bank'))));
          ...
          const payday: Payday = {
            ...
            finalStats: [
              {
                name: 'days',
                value: numberWithCommas(Number(Number(global.exports['authentication'].getPlayerInfo(player, 'hoursPlayed')).toFixed(1)))
              },
              {
                name: 'new_balance',
                value: '$' + numberWithCommas(Number(global.exports['authentication'].getPlayerInfo(player, 'bank')))
              }
            ]
          };
  
          TriggerClientEvent(`${GetCurrentResourceName()}:payday-given`, player, payday);
        });
      }
    }, 60 * 1000);
