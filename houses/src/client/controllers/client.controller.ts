...
export class Client extends ClientWithUIController {
    ...
    private updateUIData(house: House): void {
        ...
        let title: string = '';
        if (isUnowned) {
            title = `House For Sale! (Price: $${numberWithCommas(house.firstPurchasePrice)}) (#${house.id})`;
        } else {
            title = `${house.owner}'s house (#${house.id})${house.sellingPrice ? ' - For sale!' : ''}`;
        }

        SendNuiMessage(JSON.stringify({
            type: 'update',
            title,
            description: 'Houses offer you storage, security, a place to sleep, rent opportunities and more. Houses are automatically sold as a result of prolonged inactivity.',
            resource: GetCurrentResourceName(),
            buttons: [
                {
                    title: 'Purchase',
                    subtitle: `Purchase this house for $${numberWithCommas(housePrice)}.`,
                    icon: 'attach_money',
    ...
    protected onIncomingUIMessage(eventName: string, eventData: any): void {
        ...
        if (eventName === 'buttonclick') {
            ...
            switch (data.buttonId) {
                case 0: {
                    TriggerServerEvent(`${GetCurrentResourceName()}:request-purchase-house`);
                    break;
                }
                case 1: {
                    TriggerServerEvent(`${GetCurrentResourceName()}:request-become-tenant`);
                }
            }
        }
        ...
        if (eventName === 'dialogbuttonclick') {
            ...
            switch (data.dialogId) {
                case 'purchase_unowned_house': {
                    TriggerServerEvent(`${GetCurrentResourceName()}:confirm-purchase-house`, GetPlayerServerId(PlayerId()));
                    break;
                }
            }
        }
        ...
        onNet(`${GetCurrentResourceName()}:db-send-entities`, (houses: House[]) => {
            ...
            houses.forEach((house: House) => {
                ...
                this.createBlips([{
                    ...
                    title: !isOwnedByMe ? `House - ${!isUnowned ? 'Owned' : 'Unbought'}` : 'House - Your house',
                    pos: [house.entranceX, house.entranceY, house.entranceZ]
                }]);
                ...
                if (!this.actionPoints.find((actionPoint: ActionPoint) => { actionPoint.pos[0] === house.exitX && actionPoint.pos[1] === house.exitY && actionPoint.pos[2] === house.exitZ })) {
                    this.createActionPoints(
                        {
                            ...
                            action: () => {
                                ...
                                BeginTextCommandDisplayHelp("STRING");
                                AddTextComponentSubstringPlayerName("Press ~INPUT_PICKUP~ to exit the house.~n~Press ~INPUT_INTERACTION_MENU~ to open up the house menu.");
                                EndTextCommandDisplayHelp(0, false, true, 1);
