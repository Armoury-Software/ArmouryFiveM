...
function getPlayerInfo<T extends PlayerInfoType>(
  source: number,
  stat: string
): T {
  ...
  if (stat === 'hoursPlayed') {
    const computedHoursPlayed: number =
      Number(value) + computeHoursPlayed(source);
...
function computeHoursPlayed(source: number): number {
  ...
  if (authenticatedPlayers.has(source)) {
    computedHoursPlayed =
      Math.floor(
        (Math.abs(
          authenticatedPlayers.get(source).lastHoursPlayedCheck.getTime() -
            new Date().getTime()
        ) /
          (1000 * 60)) %
          60
      ) * 0.017;
    authenticatedPlayers.set(source, {
      ...authenticatedPlayers.get(source),
...
function spawnPlayer(target: number): void {
  const playerPosition: number[] = getPlayerInfo(target, 'lastLocation');
  TriggerClientEvent('authentication:spawn-player', target, playerPosition);
}
