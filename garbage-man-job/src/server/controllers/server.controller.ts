...
export class Server extends ServerController {
    ...
    private assignEvents(): void{
        ...
        onNet(`${GetCurrentResourceName()}:cp-reached`, () =>{
            ...
            exports['authentication'].setPlayerInfo(source, 'cash', Number(exports['authentication'].getPlayerInfo(source, 'cash')) + (20 + Math.floor(Math.random() * 15)), false);
            TriggerClientEvent(`${GetCurrentResourceName()}:force-showui`, source);
        });
        onNet(`${GetCurrentResourceName()}:get-job`, () => {
            global.exports['authentication'].setPlayerInfo(source, 'job', 'garbageman', false);
