...
export class Client extends ClientWithUIController {
  ...
  private updateUIData(): void {
    ...
    SendNuiMessage(
      JSON.stringify({
        ...
        title: 'Garbage Man Job',
        description:
          'Los Santos Cleaning Services keeps the Los Santos City clean.',
        ...
        buttons: [
          ...
          },
          {
            ...
            subtitle: !isAGarbageMan
              ? 'Become a garbage man'
              : 'You are already a garbage man',
            icon: 'badge',
  ...
  protected onIncomingUIMessage(eventName: string, eventData: any): void {
    ...
    if (eventName === 'buttonclick') {
      ...
      switch (data.buttonId) {
        case 0: {
          TriggerServerEvent(
            `${GetCurrentResourceName()}:start-job`,
            IsPedInAnyVehicle(PlayerPedId(), false)
          );
          this.hideUI();
          break;
  ...
  private addControllerListeners(): void {
    onNet(
      ...
      async (targetCheckpoint: { X: number; Y: number; Z: number }) => {
        ...
        if (!IsPedInAnyVehicle(PlayerPedId(), false)) {
          _spawnedVeh = await this.createVehicleAsync(
            GetHashKey('Trash'),
            -267.5868225097656,
            197.41978454589844,
            85.22119140625 + 5,
          ...
          TaskWarpPedIntoVehicle(PlayerPedId(), _spawnedVeh, -1);
        } else {
          _spawnedVeh = GetVehiclePedIsIn(PlayerPedId(), false);
        }
