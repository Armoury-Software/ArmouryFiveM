...
export class Server extends ServerController {
    ...
    RegisterCommand(
      ...
      (source: number) => {
        ...
        playerSkills.forEach((skill: Skill) => {
          global.exports['chat'].addMessage(
            source,
            skill.name + ': ' + (1 + Math.floor(skill.value))
          );
        });
      },
      false
  ...
  public updatePlayerSkill(
    ...
    const updatedPlayerSkills: Skill[] = currentPlayerSkills.map(
      (_skill: Skill) =>
        _skill.name === skill
          ? { ..._skill, value }
          : { ..._skill, value: _skill.value }
    );
  ...
  public getPlayerSkills(playerId: number): Skill[] {
    let playerSkills: Skill[] = global.exports['authentication'].getPlayerInfo(
      playerId,
      'skills'
    );
  ...
  public getPlayerSkill(playerId: number, skill: string): Skill {
    ...
    let playerSkill: Skill = playerSkills.find(
      (_skill: Skill) => _skill.name === skill
    );
  ...
  public incrementPlayerSkill(
    ...
    this.updatePlayerSkill(
      playerId,
      skill,
      Math.min(100, value + playerSkill.value)
    );
  }

  public decrementPlayerSkill(
    playerId: number,
    skill: string,
    value: number
  ): void {
    ...
    this.updatePlayerSkill(
      playerId,
      skill,
      Math.max(0, value - playerSkill.value)
    );
  }
